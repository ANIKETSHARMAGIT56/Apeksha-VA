...
const uint16_t FONT_ID_BODY_16 = 0;
float gammavalue;
...
int main()
{
    ...
    Clay_Initialize(clayMemory, (Clay_Dimensions){.width = static_cast<float>(GetScreenWidth()), .height = static_cast<float>(GetScreenHeight())}, (Clay_ErrorHandler){HandleClayErrors});
    ...
    SetTargetFPS(60);
    Apeksha::Client client("ws://localhost:18315", Apeksha::ClientOptions({.ReconnectDelay = 1000,
                                                                           .ReconnectAttempts = -1}));
    client.Open();
    Apeksha::json::Module guimod(client, {.name = "apeksha-gui",
                                    .type = "gui"});

    guimod.On("speech::start", [&](std::string data)
              {
                    std::cout<<"sfdskfsoncsdif"<<std::endl;
    ...
    std::thread tray_thread([&]
                            {
        Tray::Tray tray("My Tray", "../resources/icons/icon.png");
        tray.addEntry(Tray::Button("speak", [&]{
    guimod.Emit("speech::start");
  }));
  ...
  return 0; 
    ...
    while (!WindowShouldClose())
    {
        if (TAM == start)
        {
            ...
            windowtransparency.setTarget(200.0f, 1.0f);
            TAM = none;
        }
        else if (TAM == final)
        {
        ...
        )
        {
            CLAY_TEXT(
                CLAY_STRING(arr),
                CLAY_TEXT_CONFIG({
                    .textColor = {255.0f, 255.0f, 255.0f, gammavalue},
                    .fontId = FONT_ID_BODY_16,
                    .fontSize = font_size,
                }));
        }
